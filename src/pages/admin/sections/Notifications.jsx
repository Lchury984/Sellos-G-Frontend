...
const Notifications = () => {
  ...
  const loadNotifications = useCallback(async () => {
    try {
      ...
      let response;
      if (filter === 'no-leidas') {
        // Intentar obtener no leídas
        response = await notificationService.getUnreadNotifications();
        
        // Si el endpoint no existe (retorna null), usar fallback
        if (response === null) {
          // Obtener todas y filtrar en el frontend
          const allNotifications = await notificationService.getNotifications();
          const all = Array.isArray(allNotifications) ? allNotifications : allNotifications.data || [];
          response = all.filter((n) => !n.leida);
        }
      } else {
      ...
      setHasError(true);
      // Si hay error, mantener las notificaciones existentes o array vacío
  ...
  // Polling separado que solo se ejecuta si no hay errores
  useEffect(() => {
    if (hasError) return; // No hacer polling si hay errores
  ...
  }, [hasError, loadNotifications]);

  const handleMarkAsRead = async (id) => {
    try {
      await notificationService.markAsRead(id);
      // Si el endpoint funciona, recargar. Si no, actualizar localmente
  ...
  return (
    <>
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-gray-600">
              Mantente informado sobre las actividades del sistema
            </p>
          </div>
          {unreadCount > 0 && (
            <button
      ...
      {/* Mensaje si el backend no está disponible */}
      {hasError && (
        <div className="mb-4 bg-yellow-50 border border-yellow-200 text-yellow-800 px-4 py-3 rounded-lg text-sm">
          <p>
            El servicio de notificaciones no está disponible en el backend. 
          ...
          <div className="bg-white rounded-xl shadow-sm p-12 text-center">
            ...
            {hasError && (
              <p className="text-xs text-gray-400 mt-2">
                El backend puede no tener el endpoint de notificaciones implementado
              </p>
            )}
          </div>
        ) : (
